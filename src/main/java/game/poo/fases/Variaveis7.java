...
public class Variaveis7 extends FaseController {
	...
	public void initialize(URL location, ResourceBundle resources) {	
		...
		descricao.setText(""
				+ "Um novo setor sera identificado, mas o quadro ainda nao"
				+ " possui nenhuma letra. Declare a funcao main(). Dentro"
				+ " dela, crie uma variavel do tipo char chamada quadro"
				+ " com um valor inicial vazio. Em seguida, leia e"
				+ " armazene na mesma variavel a letra informada pelo"
				+ " operador e imprima esse valor."
		);
	}
	...
	public void chamarInterpretador() {
		...
		try {
			...
			for (int i = linhaInicio; i < linhaFinal; i++) {
				...
				if (linha.startsWith("char ")) {
					...
					if (elementos.length >= 4) {
						...
						if (nome.equals("quatro") && valor.equals("'-'")) {
				...
				if (linha.startsWith("scanf")) {
					if (!declaracaoConcluida)
						throw new IllegalArgumentException("Linha " + (i + 1) + ": declare corretamente a variavel quatro antes de ler o valor.");

					if (Interpretador.verificarScanf(linha) == 0)
						throw new IllegalArgumentException("Linha " + (i + 1) + ": o scanf esta incorreto ou nao corresponde a variavel quatro.");
				...
				if (linha.startsWith("printf")) {
					if (!entradaConcluida)
						throw new IllegalArgumentException("Linha " + (i + 1) + ": leia e armazene a identificacao antes de imprimir o valor.");

					if (Interpretador.verificarPrintf(linha) == 0)
						throw new IllegalArgumentException("Linha " + (i + 1) + ": o printf esta incorreto ou nao corresponde a variavel quatro.");
			...
			if (!declaracaoConcluida) throw new IllegalArgumentException("Crie uma variavel do tipo char chamada 'quatro' com o valor inicial '-'.");

			if (!entradaConcluida) throw new IllegalArgumentException("Leia e armazene na variavel quatro a letra informada.");

			if (!saidaConcluida) throw new IllegalArgumentException("Imprima o valor armazenado na variavel quatro.");
