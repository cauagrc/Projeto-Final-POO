...
public class Variaveis5 extends FaseController {
	...
	public void initialize(URL location, ResourceBundle resources) {	
		...
		descricao.setText(""
				+ "Um sensor registrou inicialmente uma temperatura de 18.5 graus."
				+ " Apos uma nova medicao, o operador informou a temperatura correta."
				+ " Declare a funcao main(). Dentro dela, crie uma variavel do tipo float chamada sensor"
				+ " com o valor inicial 18.5. Em seguida, leia e armazene na mesma variavel a nova"
				+ " temperatura informada pelo operador e imprima esse valor."
				);
	}
	...
	public void chamarInterpretador() {
		...
		try {
			...
			for (int i = linhaInicio; i < linhaFinal; i++) {
				...
				if (linha.startsWith("float ")) {
					...
					if (elementos.length >= 4) {
						...
						if (nome.equals("sensor") && valor.equals("18.5")) {
				...
				if (linha.startsWith("printf")) {
					if (!entradaConcluida)
						throw new IllegalArgumentException("Linha " + (i + 1) + ": leia e armazene a nova temperatura antes de imprimir o valor.");
			...
			if (!declaracaoConcluida) throw new IllegalArgumentException("Crie uma variavel do tipo float chamada 'sensor' com o valor inicial 18.5.");

			if (!entradaConcluida) throw new IllegalArgumentException("Leia e armazene na variavel sensor a nova temperatura informada.");
